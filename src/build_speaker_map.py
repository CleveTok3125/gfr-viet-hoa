...
This script extracts that mapping and writes:

  data/scenario_speakers.json
      { "<sentenceID>": { "chara": "NP0000", "voice": "VO_...", "name_ko": "루리아",
                          "listener": "1", "emotion": "0" }, ... }

It can also annotate a manual-review context file (see --ctx-in/--ctx-out)
with the speaker for each flagged line, so a maintainer can decide the
address term for the gender-selectable Captain.
...
def main():
    ...
    ap.add_argument("--ctx-in", default="/tmp/claude/opencode/captain_context.txt",
                    help="input context file (captain_context.txt)")
    ap.add_argument("--ctx-out", default=None, help="annotated context output path")
    args = ap.parse_args()
    ...
    # 2) Annotate the manual-review context file (captain_context.txt) with
    #    the speaker for each flagged line, so the maintainer can decide the
    #    address term for the Captain ("Anh" reads wrong for a female Captain).
    ctx_in = args.ctx_in or "/tmp/claude/opencode/captain_context.txt"
    ctx_out = args.ctx_out or "/tmp/claude/opencode/scenario_captain_context_speaker.md"
